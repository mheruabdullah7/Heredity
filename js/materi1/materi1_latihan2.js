  ...
  async function cekJawaban() {
    ...
    try {
      ...
      const dbRef = ref(db, '/UserSiswa/19/latihan/latihan-2/' + userId);
      // Delete old saved answers
      const oldSnapshot = await get(query(dbRef, orderByKey()));
      if (oldSnapshot.exists()) {
        const keys = Object.keys(oldSnapshot.val());
        for (const key of keys) {
          await remove(ref(db, '/UserSiswa/19/latihan/latihan-2/' + userId + '/' + key));
        }
      }
      const timestamp = Date.now();
      await set(ref(db, '/UserSiswa/19/latihan/latihan-2/' + userId + '/' + timestamp), {
        answers: answers,
        score: score,
        timestamp: timestamp
      });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const tabIds = ['AyoBerlatih2', 'AyoBerlatih3'];

  tabButtons.forEach((button, index) => {
    button.addEventListener('click', (event) => {
      openTab(event, tabIds[index]);
    });
  });
});
