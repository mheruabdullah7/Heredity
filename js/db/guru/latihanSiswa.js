...
$(document).ready(function(){
    ...
    $('#cari').click(function(){
      if($('#kelas').val() == "" || $('#latihan').val() == ""){
        Swal.fire({
          icon: 'error',
          title: 'Mohon Maaf',
          text: 'Tidak ada kelas/latihan yang dipilih',
        })
      }else{
        getDataSiswa.then(users => {
          $('#example1').DataTable().clear().destroy();
          const tampilData = document.querySelector('#dataLatihan');
          for (let user in users) {
            if (users[user].latihan) {
              if(users[user].kelas == ($('#kelas option:selected').text())){
                ...
                <tr data-id=${user}>
                  ...
                  <td class="text-center">${users[user].latihan[($('#latihan option:selected').val())].totalPoin}</td>
                  <td class="text-center">${users[user].latihan[($('#latihan option:selected').val())].waktu}</td>
                  <td class="text-center">${users[user].latihan[($('#latihan option:selected').val())].tanggal}</td>
                </tr>
              `;
            ...
            buttons: [
              {
                ...
                filename: 'Rekap '+ ($('#latihan option:selected').text())  +' Kelas '+ ($('#kelas option:selected').text()), // Nama file yang diunduh
                title: 'Rekap '+ ($('#latihan option:selected').text())  +' Kelas '+ ($('#kelas option:selected').text()), // Judul  khusus
              ...
              },
              {
                ...
                title: 'Rekap '+ ($('#latihan option:selected').text())  +' Kelas '+ ($('#kelas option:selected').text()), // Judul cetak khusus
        ...
        $('.detailLatihan').html($("#latihan").find("option:selected").text())
      }
    });
