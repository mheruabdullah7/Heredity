...
function hasil(){
  onValue(ref(db,  `UserSiswa/${nisn}/evaluasi`), (snapshot) => {
    ...
    if(data[`skor`] == ""){
      mulaiKuis.innerHTML += `
      <div class="card-body">
          <h3 class="text-center">Petunjuk Pengerjaan Evaluasi</h3>
          <hr>
          <ol>
    ...
    onValue(ref(db, "Soal/Soal_Evaluasi"), (snapshot) => {
        ...
        for (let i = 1; i <= 20; i++) {
          ...
          <div class="div-soal ${tampilanSoal} px-2">
              ...
              <div class="lg-lh text-break mx-2 soalteks" id="soalTeks${i}">${data[`Soal_Evaluasi_${i}`]["teks-soal"]}</div>
