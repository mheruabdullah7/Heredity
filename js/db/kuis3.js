...
function getUsername(){
  if(nama == null ){
    ...
    onValue(ref(db, "Soal/Soal_kuis-3"), (snapshot) => {
      // const pilihanGanda = document.querySelector(".pilihan-ganda");
        const data = snapshot.val();
...
if (pilihanGanda) {
  pilihanGanda.innerHTML += `
          <div class="div-soal ${tampilanSoal} px-2">
              <div ${data[`Soal_kuis-3_${i}`].tmpl_img}>
                  <div class="imagessKuis"><img src="${data[`Soal_kuis-3_${i}`].gambar}" class="rounded mx-auto d-block" width="20%"></div>
              </div>
  
              <div class="lg-lh text-break mx-2 soalteks" id="soalTeks${i}">${data[`Soal_kuis-3_${i}`]["teks-soal"]}</div>
          ...
          dbjawaban[i - 1] = data[`Soal_kuis-3_${i}`].kuncijwb;
