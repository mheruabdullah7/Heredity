...
function hasil(){
  onValue(ref(db,  `UserSiswa/${nisn}/kuis/kuis-3`), (snapshot) => {
    const data = snapshot.val();
    if(data[`skor`] == ""){
      mulaiKuis.innerHTML += `
      <div class="card-body">
          <h3 class="text-center">Petunjuk Pengerjaan Kuis 3</h3>
          <hr>
          <ol>
            <li class="mb-1 lh-lg">Kuis 3 terdiri dari 10 soal berupa pilihan ganda.</li>
          ...
          <hr>
          <a class="d-md-flex justify-content-md-end text-decoration-none " href="kuis3.html">
      ...
      mulaiKuis.innerHTML += `
      <div class="card-body">
        <h3 class="text-center">Hasil Kuis 3</h3>
        <hr>
        ...
        <div class="d-md-flex justify-content-md-center text-decoration-none tombol-hasil-kuis">
          <a href="/html/materi.html" class="btn btn-warning mx-2" tabindex="-1" role="button">Ulang Materi <i class="fas fa-undo-alt"></i></a>
          <button href="kuis3.html" class="btn btn-danger mx-2" id="ulangi-kuis" >Ulang Kuis <i class="fas fa-undo-alt"></i></button>
        </div>
      </div>`

      onValue(ref(db,  `kkm/kuis3/`), (snapshot) => {
...
function getUsername(){
  if(nama == null ){
    ...
    onValue(ref(db, "Soal/Soal_Kuis_3"), (snapshot) => {
        // const pilihanGanda = document.querySelector(".pilihan-ganda");
        const data = snapshot.val();
        ...
        for (let i = 1; i <= 10; i++) {
          ...
          <div class="div-soal ${tampilanSoal} px-2">
              <div ${data[`Soal_Kuis_3_${i}`].tmpl_img}>
                  <div class="imagessKuis"><img src="${data[`Soal_Kuis_3_${i}`].gambar}" class="rounded mx-auto d-block" width="20%"></div>
              </div>
  
              
              <div class="lg-lh text-break mx-2" id="soalTeks${i}">${data[`Soal_Kuis_3_${i}`]["teks-soal"]}</div>
              <label for="${i}a" class="label-${i} d-block" >
              <input class="d-none radio-pilihan-ganda-${i}" type="radio" id="${i}a" name="no${i}" value="a" />
              <div class="pilihan-jawaban pilihan-jawaban-${i}">a. ${data[`Soal_Kuis_3_${i}`].a}</div>
              </label>
              <label for="${i}b" class="label-${i} d-block">
              <input class="d-none radio-pilihan-ganda-${i}" type="radio" id="${i}b" name="no${i}" value="b" />
              <div class="pilihan-jawaban pilihan-jawaban-${i}">b. ${data[`Soal_Kuis_3_${i}`].b}</div>
              </label>
              <label for="${i}c" class="label-${i} d-block">
              <input class="d-none radio-pilihan-ganda-${i}" type="radio" id="${i}c" name="no${i}" value="c" />
              <div class="pilihan-jawaban pilihan-jawaban-${i}">c. ${data[`Soal_Kuis_3_${i}`].c}</div>
              </label>
              <label for="${i}d" class="label-${i} d-block">
              <input class="d-none radio-pilihan-ganda-${i}" type="radio" id="${i}d" name="no${i}" value="d" />
              <div class="pilihan-jawaban pilihan-jawaban-${i}">d. ${data[`Soal_Kuis_3_${i}`].d}</div>
              </label>
          
          </div>
          `;
          dbjawaban[i - 1] = data[`Soal_Kuis_3_${i}`].kuncijwb;
          MathJax.typesetPromise($('#soalTeks' + i)).catch((err) => console.log(err)); //agar mathjax berfungsi
        ...
        // Perulangan perpindahan kotak soal di klik
        for (let i = 0; i < 10; i++) {
        ...
        for (let i = 1; i <= 10; i++) {
          ...
          for (let j = 0; j < labelPilihanGanda.length; j++) {
            labelPilihanGanda[j].addEventListener("click", () => {
              ...
              // console.log("label " + pilihan[j]);
              jawabanSiswa = pilihan;
              jawabanSiswaSemua[i] = pilihan;
  
              set(ref(db, `UserSiswa/${nisn}/kuis/kuis-3/nomor/${i}`), {
                jawaban: pilihan,
              });
            });
          }
        ...
        btnHome.addEventListener("click", function(){
          ...
          .then((willDelete) => {
            if (willDelete) {
              location.href = "materi3_kuis.html";
        ...
        onValue(ref(db, `UserSiswa/${nisn}/kuis/kuis-3/nomor/`), (snapshot) => {
          ...
          function checkEverySecond() {
            ...
            if(siswaWaktu == null){
              ...
              for(let j = 0; j < dbjawaban.length; j++){
                // console.log(dbjawaban[j] + " db");
                if (dbjawaban[j] == siswajwb[j]){
                  // console.log("kunci jwb " + [j] +" benar");
                    benar += 1;
                }
              }

              update(ref(db, `UserSiswa/${nisn}/kuis/kuis-3`), {
                selesai: true,
                skor: benar * 10,
                tanggal: dbtanggal,
                waktu: dbwaktu
